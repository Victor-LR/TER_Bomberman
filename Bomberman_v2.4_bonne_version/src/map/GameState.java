	...
	public void bombExplode(Objet_Bomb bomb, Agent_Bomberman bomberman )
	{
		...
		for(int i = x; i<= range_limit; i++){
				...
				for(int j = 0; j<bombermans.size(); j++){
					...
					if(bomberman1.getX() == i & bomberman1.getY() == y & bomberman.getId() != bomberman1.getId() ){
						bombermans.remove(j);
		...
		for(int i = y; i<= range_limit; i++){
			...
			for(int j = 0; j<bombermans.size(); j++){
				Agent_Bomberman bomberman1 = bombermans.get(j);
				if(bomberman1.getX() == x & bomberman1.getY() == i  & bomberman.getId() != bomberman1.getId() ){
					bombermans.remove(j);
					bomberman.setPoints(bomberman.getPoints() + 500);
				}
			}
			...
			for(int i = 0; i < bombermans.size(); i++){
				...
				for (int j = 0; j < items.size(); j++){
					...
					if ( (bomberman.getX() == item.getObjX()) && (bomberman.getY() == item.getObjY()) ){
						...
						else if((item.getType() == ObjetType.BOMB_DOWN) && bomberman.getNbBombes() > 1) bomberman.setNbBombes(bomberman.getNbBombes()-1);
						else if((item.getType() == ObjetType.FIRE_SUIT)) {
							bomberman.setInvincible(true);
							bomberman.setEtatInv(0);
							
						}else if((item.getType() == ObjetType.SKULL)) {
				...
				/*if (bomberman.isSick())
					if(bomberman.getEtatSick() <=20) bomberman.setEtatInv(bomberman.getEtatInv()+1);
					else bomberman.setSick(false);*/
				...
				else if (bomberman.getNbActions() > 2){
